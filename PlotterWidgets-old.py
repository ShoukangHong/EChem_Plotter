...
class MngTabWidget(MngTabWindow):
    ...
    def excecuteAction(self,action):
        func = ACTIONDICT[action['action']]['func']
        print(action)
    ...
    def saveMethod(self):
        options = QFileDialog.Options()
        options |= QFileDialog.DontUseNativeDialog
        fileName, _ = QFileDialog.getSaveFileName(self,"QFileDialog.getSaveFileName()", "method.txt","Method Files (*.txt)", options=options)
        if fileName:
            actions = '['
            for x in range(self.actList().count()-1):
                action = self.actList().item(x).statusTip()
    ...
    def openMethod(self):
        options = QFileDialog.Options()
        options |= QFileDialog.DontUseNativeDialog
        fileName, _ = QFileDialog.getOpenFileName(self,"QFileDialog.getOpenFileName()", "","Method Files (*.txt)", options=options)
        if fileName:
            try:
